    ...
    return (
        <>
            ...
            <div className="flex gap-4 p-2 border-2 rounded-2xl">
                {isLogin && (<Link href={''} className="p-2 text-2xl border rounded-xl">Logout</Link>)}
                {!isLogin && (<Link href={'/Auth'} className="p-2 text-2xl border rounded-xl">Login</Link>)}
                {isLogin && userData.role === "Admin" && (<Link href={''} className="p-2 text-2xl border rounded-xl">Admin Panel</Link>)}
            </div>
        </>
    )
}
